                ...
                {cart.map(item => (
                  <div key={item.id} className="flex gap-4 items-center">
                    <div className="w-16 h-16 bg-gray-50 rounded-xl p-2 shrink-0 border border-gray-100">
                      <img src={item.image_url ? (item.image_url.startsWith('http') ? item.image_url : `${API_BASE_URL}admin/${item.image_url}`) : 'https://images.unsplash.com/photo-1593095948071-474c5cc2989d?auto=format&fit=crop&q=80&w=600'} alt={item.name} className="w-full h-full object-contain mix-blend-multiply" />
                    </div>
